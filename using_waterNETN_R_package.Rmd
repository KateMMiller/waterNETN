...
Get N parameters for all sites and non-QAQC events in MIMA. <br>
...
Get ChlA for all events, including QC samples, in 2023 for Jordan Pond in ACAD
```{r}
...
Get surface-only measurements, defined as sampled within 2m of the surface for ACAD lakes from 2021:2023. If more than one measurement exists, the value represents the median of all samples within 2m of the surface.
```{r}
...
#### getDischarge()
This function allows you to query the Discharge_Data view by park, site, site type, year, month, measurement method (e.g., flowtracker or pygmy), and measurement rating (e.g., E, G, F, P). 

Get discharge data for all LNETN sites and years measured with flowtracker and return all columns
```{r}
lnetn <- c("MABI", "MIMA", "MORR", "ROVA", "SAGA", "SAIR", "SARA", "WEFA")
ft <- getDischarge(park = lnetn, method = "LNETN Pygmy", output = 'verbose')
print_head(ft)
```

Get discharge data for all ACAD streams with measurement rating of Excellent or Good and return reduced columns
```{r}
