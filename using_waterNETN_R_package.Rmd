...
#### getChemistry()
This function allows you to query the Chemistry_Data view by park, site, site type, year, month, parameter, sample type, and sample depth type (i.e., surface vs. below surface). The returned data frame is long (i.e. stacked) to facilitate data summary and plotting. Surface only measurements, defined as as the medium of all samples within 2m of the surface, are the default.
...
```{r}
surf <- getChemistry(park = "ACAD", site_type = "lake", years = 2021:2023, sample_depth = 'all')
print_head(surf)
```
...
It works with both lab chemistry data and Sonde in situ data. If multiple sites are specified, they will be plotted on the same figure. If multiple parameters are specified, they will be plotted on separate figures.

<p class = 'fxn'>Plot smoothed surface pH for Eagle Lake for all years, removing the legend and using default span of 0.3. </p>
```{r}
plotTrend(site = "ACEAGL", parameter = "pH", legend_position = 'none')

```
<p class = 'fxn'>Plot smoothed surface pH for Eagle Lake for all years, removing the legend and using span of 0.75. </p>
```{r}
plotTrend(site = "ACEAGL", parameter = "pH", legend_position = 'none', span = 0.75)

```

<p class = 'fxn'>Plot non-smoothed surface pH for Eagle Lake for 2023. </p>
```{r}
plotTrend(site = "ACEAGL", parameter = "pH", years = 2023, smooth = FALSE)

```

<p class = 'fxn'>Plot smoothed surface pH for all SARA streams over all years with 0.6 span. </p>
```{r}
plotTrend(park = "SARA", site_type = "stream", 
