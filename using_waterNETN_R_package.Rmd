...
```{r}
prim <- getClimDaymet(site = "MORRSB", years = 2006:2023, export = T, filepath = "./data")
...
```{r}
rova_ws <- getClimWStat(park = "ROVA", years = 2013:2023, export = T, filepath = "./data")
print_head(rova_ws)
```
...
```{r}
brbk <- getChemistry(site = "ACBRWN", QC_type = "all", param = "all", years = 2023)
print_head(brbk)
```
...
```{r}
ACAD_lake <- getSondeInSitu(site = 'ACJORD', parameter = "Temp_C", sample_depth = "all")
```
...
```{r message = FALSE, warning = FALSE}
plotTrend(site = "ACWHOL", parameter = "SO4_ueqL", smooth = F, include_censored = TRUE, legend_position = 'bottom')
...
```{r message = FALSE, warning = FALSE}
plotTrend(site = "ACJORD", parameter = c("SDepth_m", "PenetrationRatio", "pH", "DOC_mgL"), span = 0.75, color_theme = 'set1')
...
```{r message = FALSE, warning = FALSE}
plotTrend(site = "MABIPA", parameter = "WaterLevelFeet", span = 0.75, color_theme = 'set1')
...
```{r message = FALSE, warning = FALSE}
plotTrend(park = "MORR", parameter = c("TN_mgL", "TP_ugL", "SO4_ueqL"), span = 0.6, legend_position = 'bottom', color_theme = 'accent')
```
...
<p class = 'fxn'>Plot temperature for Eagle Lake for years 2013 - 2023 corrected by elevation. </p>
```{r message = FALSE, warning = FALSE}
plotLakeProfile(site = "ACEAGL", parameter = "Temp_C", depth_type = 'elev', years = 2013:2023)

```

<p class = 'fxn'>Plot temperature for Eagle Lake for years 2013 - 2023 with raw sample depth. </p>
```{r message = FALSE, warning = FALSE}
plotLakeProfile(site = "ACEAGL", parameter = "Temp_C", depth_type = 'raw', years = 2013:2023)

```

<p class = 'fxn'>Plot temperature for all ACAD lakes sampled in 2023 and raw sample depth. </p>
```{r message = FALSE, warning = FALSE}
lakes23 <- c("ACBUBL", "ACEAGL", "ACECHO", "ACJORD", "ACLONG", "ACROUN", "ACSEAL", "ACUBRK", "ACUHAD", 
           "ACWHOL", "ACWOOD")
plotLakeProfile(park = "ACAD", site = lakes23, parameter = "Temp_C", depth_type = 'raw', years = 2023)

```

<p class = 'fxn'>Plot DO all ACAD lakes sampled in 2023 and raw sample depth, using reversed ryb palette. </p>
```{r message = FALSE, warning = FALSE}
...
p class = 'fxn'>Plot pH for Seal Cove Pond from 2013 to 2023 and sample elevation, using spectral palette and adding site name as title. </p>
```{r message = FALSE, warning = FALSE}
plotLakeProfile(site = "ACSEAL", parameter = "SpCond_uScm", 
                depth_type = 'elev', years = 2013:2023, 
                color_theme = 'spectral', title = TRUE)

```

p class = 'fxn'>Plot pH for Jordan Pond from 2013 to 2023 and sample elevation, using reversed spectral palette. </p>
```{r message = FALSE, warning = FALSE}
plotLakeProfile(site = "ACJORD", 
                parameter = "pH", depth_type = 'elev', years = 2013:2023, 
                color_theme = 'spectral', color_rev = TRUE)
...
<p class = 'fxn'>Combine plots for temp, DO, pH, and conductance using the `cowplot` package.</p>
