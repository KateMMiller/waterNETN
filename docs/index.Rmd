...
```{r}
climateNETN::plotClimTrend(park = "LNETN", years = 1895:2024, layers = 'smooth', 
...
```{r}
climateNETN::plotClimTrend(park = "ACAD", years = 1895:2024, parameter = c("tmin", "tmean", "tmax"), span = 0.1, layers = 'smooth', facet_param = F, legend_position = 'right', palette = "Set2")
```
...
```{r}
climateNETN::plotClimTrend(park = "MABI", years = 2006:2023, parameter = "all", layers = c("points", "lines"), facet_param = T, gridlines = "grid_y", units = 'eng')
```
...
```{r}
climateNETN::plotClimTrend(park = "MABI", years = 2006:2023, parameter = "all", layers = 'smooth', span = 0.7, palette = "Dark2")
```
...
```{r}
climateNETN::plotClimAnom(park = c("ACAD"), years = 2004:2024, parameter = c("tmean", "ppt"), 
...
```{r}
climateNETN::plotClimCumPrecip(park = "ACAD", years = 2014:2024, legend_position = 'bottom', months = 4:10)
...
```{r}
climateNETN::plotClimDrought(park = c("ACAD"), years = 2023, gridlines = "grid_y")
```

<p class='fxn'>Plot drought info for MABI and SAGA for 2020 to 2024 with gridlines on x axis.</p>
```{r}
climateNETN::plotClimDrought(park = c("MABI", "SAGA"), years = 2020:2024, gridlines = "grid_x")
```

<p class='fxn'>Plot drought info for both MORR counties for 2023</p>
```{r}
climateNETN::plotClimDrought(park = "MORR", years = 2023, dom_county = FALSE)
```

<p class="fxn">Plot drought info for both ACAD counties in 2020 for May through October </p>
```{r message = F, warning = F}
climateNETN::plotClimDrought(park = "ACAD", years = 2020, dom_county = FALSE, months = 5:10)
```

<p class="fxn">Plot drought info for LNETN in 2023 with legend on bottom </p>
```{r message = F, warning = F}
climateNETN::plotClimDrought(park = "LNETN", years = 2023, legend_position = 'bottom')
```
