...
<p class = 'fxn'>Plot avg temp for LNETN from 1895:2024, without points and with smoothed line and span 0.1 and gridline on y-axis, and in English units.</p>
```{r}
...
```{r}
plotClimAnom(park = "ACAD", years = 1895:2024, parameter = "tmean", 
             legend_position = 'right', gridlines = "grid_y", units = "eng")
```
...
plotClimAnom(park = c("ACAD"), years = 2004:2024, parameter = c("tmean", "ppt"), 
             legend_position = 'bottom', averages = "norm1990")
```
...
<p class = 'fxn'>Plot mean monthly temp for MABI for 2019:2023 and all months with red-blue color palette and the 4 decadal averages with gridlines on y in english units.</p>
```{r}
...
plotClimComps(park = "MABI", years = 2019:2023, parameter = "tmean", palette = c("red", "blue"), 
              layers = c('points', 'lines'), averages = "norm1990", units = 'eng')
```
<p class = 'fxn'>Plot max monthly temp for ROVA for 2023 and 2024, with 'viridis' palette and 20th century norm (default) using scientific units.</p>
...
plotClimRel(park = "MABI", years = 2024, parameter = "temp", units = "eng",
