...
<li> Choose color palette via `color_theme`. Default is 'viridis', but other options are 'set1', 'dark2', and 'accent'. If there are other color palletes you'd like me to add, let me know. The only thing I'm trying to avoid is creating the palette manually, since number of sites could vary by plot. </li>
...
<p class = 'fxn'>Plot smoothed surface pH for Eagle Lake for all years using default span of 0.3 and by default not including the legend. </p>
```{r message = FALSE, warning = FALSE}
plotTrend(site = "ACEAGL", parameter = "pH", color_theme = 'dark2')
...
plotTrend(park = "MORR", parameter = c("TN_mgL", "TP_ugL", "SO4_ueqL"), 
          span = 0.6, legend_position = 'bottom', color_theme = 'accent')
```
...
Next step with lake profiles is to model them by day of year, so we can plot a consistent interval between samples, rather than just use the month to plot, like here. This would allow samples taken twice within a month to both be counted, and for early vs. late in the month samples to be corrected for. That will take a bit to figure out...
...
<p class = 'fxn'>Plot temperature for Eagle Lake for years 2006 - 2023 with raw sample depth, and no plot title or thermocline. Note that I can go back to 2006 because I'm using raw sample depth instead of elevation.</p>
```{r message = FALSE, warning = FALSE}
...
<p class = 'fxn'>Plot DO all ACAD lakes sampled in 2023 and raw sample depth, using reversed ryb palette. Temperature thermocline is also included as black circles.</p>
```{r message = FALSE, warning = FALSE}
...
pHplot <- plotLakeProfile(site = sitecode, parameter = "pH", depth_type = depth,
                          years = year, months = mon, color_theme = "ryb", color_rev = T,
                          plot_title = ptitle)

cnplot <- plotLakeProfile(site = sitecode, parameter = "SpCond_uScm", depth_type = depth,
                         years = year, months = mon, color_theme = 'rb',
                         plot_title = ptitle)
...
#### plotPrecipDischarge()
This function produces a plot with dual y-axes, one for precipitation an done for discharge. The x-axis is date. This function only works for one stream monitoring site at a time. Note that ggplot tends to have a lot of warnings that are hard to suppress, particularly for this plot, which has a daily value for precipitation and only monthly values for discharge. You're always going to get a message about that by using this function. Function currently only plots years where discharge is collected.
...
#### plotClimTrend()
This function produces a line or smoothed trend plot filtered on park, site, year, month, and climate parameter. If multiple sites are specified, they can either be plotted on the same figure or separate figures. If multiple parameters are specified, they will be plotted on separate figures. Note that all months (1:12) are default. If you only want sample months, specify months = 5:10.

<p classs = 'fxn'>Plot monthly weather station data for Pogue Stream in MABI from 2006:2023 (all months is default), without a smoothed line.</p>
```{r}
plotClimTrend(site = "MABIPA", years = 2006:2023, parameter = "ws_ppt_mm", smooth = F)
```
<p classs = 'fxn'>Plot monthly weather station data for Pogue Stream in MABI from 2006:2023, with smoothed line and span 0.7.</p>
```{r}
plotClimTrend(site = "MABIPA", years = 2006:2023, parameter = "ws_ppt_mm", span = 0.7)
```


<p classs = 'fxn'>Plot monthly Daymet mean temperature for Pogue Stream in MABI and Kroma Kill in SARA, and  from 2006:2023, with smoothed line, span 0.7, and only sample months.</p>
```{r}
plotClimTrend(site = c("MABIPA", "SARASA"), years = 2006:2023, parameter = "dm_tmean_C", span = 0.7, months = 5:10)

head(getClimWStat(park = "SARA", years = 2006:2023))
head(sumClimMonthly(park = "SARA", years = 2006:2023, data_type = "wstn"))
```


### Summary Functions {.tabset}
#### sumClim30yr()
This function calculates monthly 30 year norms from daily Daymet data by site. User can specify start of the 30-year norm, the earliest being 1980. Note that Daymet data are downloaded from an online web service and can be slow if many sites selected. Normals returned are 30-year monthly averages for total precipitation in mm (ppt_mm), average maximum temperature (tmax_C), average minimum temperature (tmin_C), and average temperature (tmean_C). Note that occasionally you're unable to connect to the server and will receive and error message when that happens.

<p class = 'fxn'>Summarize 1980 - 2009 30-year normals for all MIMA sites.</p>
```{r}
mima80 <- sumClim30yr(park = "MIMA", start_year = 1980)
print_head(mima80)
```

<p class = 'fxn'>Summarize 1990 - 2019 30-year normals for Weir Pond.</p>
```{r}
wefa90 <- sumClim30yr(site = "WEFAPA", start_year = 1990)
print_head(wefa90)
```
