...
Note that soon I'm going to strip most of these climate functions out of this package and create a new climateNETN package, so any protocol can work with these data. The function names won't change, and you won't need to update your code beyond needing to install and load the separate package. I'll update this website once I'm finished with that transition.
...
```{r cache = T}
plotClimComps(park = "ROVA", years = 2023:2024, parameter = "tmax", palette = "viridis", layers = 'lines')
```
...
```{r message = F, warning = FALSE, cache = T}
