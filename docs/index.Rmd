...
<li> Choose color palette via `color_theme`. Default is 'viridis', but other options are discrete palettes in RColorBrewer, including 'Set1', 'Dark2', and 'Accent'. </li>
...
#### plotClimComps()
This function compares decadal (1980 - 1989, ..., 2010 - 2019) average monthly climate variables with user-specified years to provide an idea of how extreme or normal a given month in a year is. This function currently only works with Daymet data, so is not immediately available for the current date.

<details open><summary class='drop'>Single parameter</summary>
<p class = 'fxn'>Plot mean monthly temp for the Pogue in MABI for 2019:2023 and all months with red-blue color palette.</p>
```{r}
plotClimComps(site = "MABIPA", years = 2019:2023, parameter = "temp_mean", color_theme = "RdBu")
```
<p class = 'fxn'>Same as above, but with points too.</p>
```{r}
plotClimComps(site = "MABIPA", years = 2019:2023, parameter = "temp_mean", color_theme = "RdBu", layers = c('points', 'lines'))
```
<p class = 'fxn'>Plot max monthly temp for Eagle Lake and Jordan Pond for 2019:2023 and sample months only, with 'viridis' palette</p>

```{r}
plotClimComps(site = c("ACEAGL", "ACJORD"), years = 2019:2023, months = 5:10,
              parameter = "temp_max", color_theme = "viridis", layers = 'lines')
```
```{r}
# Plot total monthly precip for Jordan Pond for past 10 years using red-yellow-blue color scheme
plotClimComps(site = "ACJORD", years = 2013:2023, parameter = 'precip', color_theme = "RdYlBu")
```
</details><br>

<details open><summary class = 'drop'>Multiple plots</summary>
```{r}
tmax <- plotClimComps(site = "ACEAGL", years = 2013:2023, parameter = "temp_max", months = 5:10,
                      layers = 'lines', legend_position = 'none', color_theme = "Blues", color_rev = T)

tmin <- plotClimComps(site = "ACEAGL", years = 2013:2023, parameter = "temp_min", months = 5:10, 
                      layers = 'lines', legend_position = 'none', color_theme = "Blues", color_rev = T)
tmean <- plotClimComps(site = "ACEAGL", years = 2013:2023, parameter = "temp_mean", months = 5:10, 
                       layers = 'lines', legend_position = 'none', color_theme = "Blues", color_rev = T)
prec <- plotClimComps(site = "ACEAGL", years = 2013:2023, parameter = 'precip', months = 5:10, 
                      layers = 'lines', legend_position = 'none', color_theme = "Blues", color_rev = T)

prec_leg <- plotClimComps(site = "ACEAGL", years = 2013:2023, parameter = 'precip', months = 5:10, 
                          layers = 'lines', legend_position = 'right', color_theme = "Blues", 
                          color_rev = T) +
...
grid1 <- cowplot::plot_grid(tmean, tmax, NULL, tmin, prec, NULL, 
                            nrow = 2, ncol = 3, rel_widths = c(1, 1, 0.1, 1, 1, 0.1))
grid2 <- cowplot::plot_grid(grid1, NULL, leg, ncol = 3, nrow = 1, rel_widths = c(1,0.05, 0.1))
grid2
```
</details>
